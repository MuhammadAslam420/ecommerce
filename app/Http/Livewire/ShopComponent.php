...
class ShopComponent extends Component
{
    ...
    public function render()
    {
        //dd($this->used);
        if ($this->sorting == '1') {
            $products = Product::whereBetween('price', [$this->min_price, $this->max_price])->where('featured', '1')->
                where('product_type', $this->new)->where('product_type', $this->used)->where('product_type', $this->refurnished)->orderBy('created_at', 'DESC')->paginate($this->pagesize);
        } elseif ($this->sorting == 'date') {
            $products = Product::whereBetween('price', [$this->min_price, $this->max_price])->
                where('product_type', $this->new)->where('product_type', $this->used)->where('product_type', $this->refurnished)->orderBy('created_at', 'DESC')->paginate($this->pagesize);
        } elseif ($this->sorting == 'price') {
            $products = Product::whereBetween('price', [$this->min_price, $this->max_price])->
                where('product_type', $this->new)->where('product_type', $this->used)->where('product_type', $this->refurnished)->orderBy('price', 'ASC')->paginate($this->pagesize);
        } elseif ($this->sorting == 'price-desc') {
            $products = Product::whereBetween('price', [$this->min_price, $this->max_price])->
                where('product_type', $this->new)->where('product_type', $this->used)->where('product_type', $this->refurnished)->orderBy('price', 'DESC')->paginate($this->pagesize);
        } elseif ($this->sorting == 'az') {
            $products = Product::whereBetween('price', [$this->min_price, $this->max_price])->
                where('product_type', $this->new)->where('product_type', $this->used)->where('product_type', $this->refurnished)->orderBy('name', 'ASC')->paginate($this->pagesize);
        } elseif ($this->sorting == 'za') {
            $products = Product::whereBetween('price', [$this->min_price, $this->max_price])->
                where('product_type', $this->new)->where('product_type', $this->used)->where('product_type', $this->refurnished)->orderBy('name', 'DESC')->paginate($this->pagesize);
        ...
        $seo = Seo::where('page_name', 'shop')->firstOrFail();
        try {
            return view('livewire.shop-component', ['products' => $products])->layout("layouts.base", ['seo' => $seo]);
        } catch (\Exception $e) {
